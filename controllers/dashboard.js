...
function withdrawalStatusToActivityType(status) {
    ...
    if (s === "approved" || s === "success" || s === "successful" || s === "completed") {
        return "withdrawal_approved";
    }
    if (s === "rejected" || s === "declined" || s === "failed" || s === "reversed") {
...
export const collectionActivities = async (req, res) => {
    ...
    try {
        ...
        const [contribRes, withdrawalRes] = await Promise.all([
            ...
            supabase
                .from('withdrawals')
                .select('id, collection_id, amount, status, created_at, updated_at, destination_account')
                .in('collection_id', collectionIds)
        ...
        const withdrawalActivities = (withdrawalRes.data || []).map((w) => {
            ...
            return {
                ...
                account_number: destination.accountNumber || destination.account_number || null,
                account_name: destination.accountName || destination.account_name || null,
            };
        });
